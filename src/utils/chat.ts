...
const validateAndFixChatConfig = (config: ConfigInterface) => {
    ...
    if (!config.width) config.width = defaultChatConfig.width;
    if (!(typeof config.width === 'number')) return false;

    if (!config.height)
        config.height = defaultChatConfig.height;
    if (!(typeof config.height === 'number')) return false;

    if (!config.num_inference_steps) config.num_inference_steps = defaultChatConfig.num_inference_steps;
    if (!(typeof config.num_inference_steps === 'number')) return false;

    if (!config.guidance_scale) config.guidance_scale = defaultChatConfig.guidance_scale;
    if (!(typeof config.guidance_scale === 'number')) return false;
