...
import {MessageButton} from "@components/Chat/ChatContent/Message/MessageContent";

const DownloadChat = React.memo(
    ({saveRef, iconOnly}: { saveRef: React.RefObject<HTMLDivElement>, iconOnly?: boolean }) => {
        const {t} = useTranslation();
        const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
        return (
            <>
                {iconOnly ?
                    <MessageButton icon={<DownloadIcon/>} onClick={() => {
                        setIsModalOpen(true);
                    }}/> :
                    <button
